...
在持续补位模型建立前，团队执行过 1/15/20/25/30/50/100 场断点测试。一个虚拟玩家代表一场独立权威战斗和一条 WSS；历史样本通常在 86–106 秒终局，优化后的确定性客户端常在 57–65 秒终局，单场曾观察到 172 个敌人。由于终局后没有补位，这些结果只能说明当时软件、数据和实例状态下的短时断点，不能覆盖 8.3 的 Stage33 长稳结论。

| 历史阶段 | 并发 | 成功 | P95 | 当时判定 |
|---|---:|---:|---:|---|
| 初始容量基准 | 15 | 15/15 | 164.5 ms | 建议上限 |
...
| 持久暂停 + AOI pass3 | 15 | 15/15 | 153.7 ms | 当时软上限依据 |

旧门槛曾把 P95 180–200 ms、API CPU 持续超过 70% 或 P99 超过 200 ms 标为黄色并停止提高准入；30/50/100 场超过当时可接受范围。分支原文的 pass3 20 场记录被字面量截断占位符破坏，因此不把残缺行当作精确证据；可恢复的历史结论是 15 场软上限、20 场临时硬上限。当前准入必须使用持续补位和独立业务/资源红线重新判定。
...
10. 当前输入和可靠命令缓存；每 10 Tick 持久化检查点，终局/flush 立即写；API、实时模拟和 Replay Worker 可按部署目标分角色。

早期可直接对比的 15 场样本中，组合优化让接收流量下降 90.0%、P95 改善 7.0%、整机平均 CPU 下降 23.0%，当时的全绿边界从 15 场提高到 20 场（33%）。30 场本地确定性基准的托管分配从 393,261,264 B 降到 235,168,120 B（40.2%），规范状态 hash 完全相同；这些是组合收益，不能拆成单项收益，短基准 CPU 也不能替代持续补位 ECS 结果。
...
| 单机 40+ 场 | Stage33 的 40 场首轮业务成功但 API 峰值 98.86%，45/50 未执行；更早的 100 场断点也失败 | 先优化 presentation 分配、持久化和移动事务峰值，再从 30 场长稳重新走门槛；若目标仍为 100 场，评估战斗分片/多 Worker 并在获批的目标实例上重测，不得先承诺 |
