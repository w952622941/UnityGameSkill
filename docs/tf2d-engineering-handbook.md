...
版本：1.2　事实审计日期：2026-07-25（Asia/Shanghai）　适用项目：Unity 2022.3 LTS + ASP.NET Core + PostgreSQL + Docker
...
| 性能 | 第一轮结果等价算法、持久暂停/AOI 已进入 main；Stage33 另有活动注册表、常驻模拟器、有界 lane/持久化槽、池化编码和数据库热路径优化 | 两阶段都有效，但 Stage33 高级候选仍无可重建的 main 提交，不能混为同一源码事实 |
...
- 4 核 8 GiB 的 Integration ECS 不是生产容量承诺。Stage33 镜像通过 30 × 180 秒和 35 场资格轮 + 三轮重复，但 35 最差 API 峰值 84.63%、40 首轮峰值 98.86%；运营建议按 30 场，40/45/50 不认证。当前 `main` 只确认包含第一轮零规则优化、持久暂停和 AOI；Stage33 高级候选无可重建提交，不得描述为当前 `main` 能力。
