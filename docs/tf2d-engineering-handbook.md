...
版本：1.0　事实审计日期：2026-07-22（Asia/Shanghai）　适用项目：Unity 2022.3 LTS + ASP.NET Core + PostgreSQL + Docker
...
- 建立不进入 APK 的压测工具和模拟基准工具；完成 1/15/20/25/30/50/100 并发断点测试、零规则变更优化、AOI 算法剖析和容量边界判断。
...
| 性能 | 20 Hz 模拟、10 Hz 普通快照、紧急快照旁路、delta/gzip、批量持久化、空间哈希粗筛 | 规则哈希等价与 ECS 压测验证 |
...
- 4 核 8 GB 的 Integration ECS 不是最终生产容量承诺。当前稳定建议仍是 15 场软上限、20 场硬上限；单机 100 场必须先完成调度分片/多 Worker，再在目标实例重测。
...
一个虚拟玩家代表一场独立权威战斗和一条 WSS：创建独立游客身份、登录、开战、选技能、10 Hz 移动输入、接收 10 Hz 普通快照、真实刷怪/技能/投射物/掉落、死亡/结算或到时退出。服务端仍 20 Hz。历史稳定样本平均 86–106 秒；优化后确定性客户端常在 57–65 秒终局；单场曾观察到 172 个敌人。

### 8.3 容量门槛

- 绿：成功率 100%，P95 < 180 ms，API CPU 通常 < 70%，峰值 < 85%。
- 黄：P95 180–200 ms、API CPU 持续 > 70% 或 P99 > 200 ms；不提高准入。
- 红：一致性错误、失败客户端、P95 > 200 ms、持续饱和、容器重启/OOM/数据库错误；停止上探或回滚。

### 8.4 关键实测

| 阶段 | 并发 | 成功 | P95 | 结论 |
|---|---:|---:|---:|---|
| 初始容量基准 | 15 | 15/15 | 164.5 ms | 当时建议上限 |
| 初始容量基准 | 20 | 20/20 | 188.8 ms | 质量边界 |
| 初始容量基准 | 25 | 25/25 | 216.0 ms | 不通过 |
| 零规则优化 pass1 | 20 | 20/20 | 158.3 ms | 边界通过，重复 100/100 成功 |
| 零规则优化 pass1 | 25 | 25/25 | 202.3 ms | 红线 |
| 持久暂停 + AOI pass3 | 15 | 15/15 | 153.7 ms | 当前软上限依据 |
| 持久暂停 + AOI pass3 | 20 | 20/20 | 200.5 ms | 当前…285 tokens truncated…依赖实体 ID 单调有序不变量，查找改二分、快照直接复制。
7. 20 Hz 玩法不变，普通快照改 10 Hz；技能候选/终局立即发送。
8. exact-base prefix/suffix delta + 协商 gzip；保留旧 JSON 兼容。
9. 复用序列化缓冲、每客户端只保留最新待发快照、减少热路径日志。
10. 当前输入和可靠命令缓存；每 10 Tick 持久化检查点，终局/flush 立即写；API/实时模拟/Replay Worker 可分角色。

直接可比的 15 场中，接收流量下降 90.0%，P95 改善 7.0%，整机平均 CPU 下降 23.0%，全绿容量从 15 增至 20（33%）。这是组合收益，不能拆成每项独立收益。30 场本地确定性基准的托管分配从 393,261,264 B 降至 235,168,120 B（40.2%），规范状态 hash 完全相同；短基准 CPU 受噪声影响，因此未虚构 CPU 提升。
...
| 单机 100 场 | 当前 4 核断点失败 | 战斗分片/多 Worker，目标 8 核重测，不先承诺 |
