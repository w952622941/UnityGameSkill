...
- 这是一个给 AI 和团队成员复用的 Unity 项目启动知识库。内容来自TF_2D的真实执行与真机验证。
- TF_2D是一个大型的商业化网络游戏项目，采用即时战斗，这意味着这个项目对联网和并发有极高的要求。
- 我在开发这个游戏的过程中，将游戏的美术、技术完全交给了AI，包括云服务器的搭建、并行、压力测试等等，而我只负责设计。

## 内容包含

- Git 基线：AI会自动为项目创建GitHub私人仓库，并通过你指定的本地github仓库管理软件来管理这个私人仓库。
- Unity 目录重构：以大幅度提高token使用效率和大量节省token为目标，重构你的unity游戏项目，最终可以在开发过程中节省80%的token。
- 美术资源管理：如果你不知道如何让AI自动管理一个商业化游戏的美术资源，这里可以帮到你。
- 本地化开发以及手机网游的客户端/服务器高并发架构、确定性战斗、反作弊、结算、运营数据和云迁移。
- 如果你不知道如何搭建高并发服务器联网游戏，这个项目可以帮到你，为你节省大量时间。
